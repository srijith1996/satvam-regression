...
class ModelTree(object):
    ...
    def export_graphviz(self, output_filename, feature_names,
        ...
        def build_graphviz_recurse(node, parent_node_index=0, parent_depth=0, edge_label=""):
            ...
            # compute and print to tree ranges of X's
            (X_, y_) = node["data"]
            max_X = np.amax(X_, axis=0)
            min_X = np.amin(X_, axis=0)

            print max_X, min_X

            range_str = "\\n".join(["{:.2f} <= {} <= {:.2f}".format(xmin, feature_names[i], xmax)
                                            for (i, (xmin, xmax)) in enumerate(zip(min_X, max_X))])

            label_str = "{}{}\\nsamples = {}\\nmse = {:.6f}".format(threshold_str, range_str,
